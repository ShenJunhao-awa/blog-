...
import { getStorageObjectWithContext, putStorageObjectWithContext } from "../utils/storage";
...
export function StorageService(): Hono {
    ...
    app.post('/', async (c: AppContext) => {
        ...
        try {
            const result = await profileAsync(c, 'storage_put', () => putStorageObjectWithContext(c, hashkey, file, file.type, new URL(c.req.url).origin));
...
export function BlobService(): Hono {
    ...
    app.get("/*", async (c: AppContext) => {
        ...
        try {
            const response = await profileAsync(c, "blob_fetch", () => getStorageObjectWithContext(c, decodeURIComponent(key)));
