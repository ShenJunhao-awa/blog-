...
describe('StorageService', () => {
    ...
    describe('GET /blob/* - Stream file', () => {
        ...
        it('should store and retrieve file through database when STORAGE_MODE is database', async () => {
            ...
            const dbApp = createAppWithEnv(dbEnv, 1);
            const formData = new FormData();
            formData.append('key', 'test.txt');
            formData.append('file', new File(['test content'], 'test.txt', { type: 'text/plain' }));

            // Upload file
            const uploadRes = await dbApp.request('/', {
                method: 'POST',
                body: formData,
            }, dbEnv);
            ...
            expect(uploadPayload.url).toMatch(/^https:\/\/images\.example\.com\/images\/[a-f0-9]+\.txt$/);

            // Get the stored key from the response URL
            const urlParts = uploadPayload.url.split('/');
            const storedKey = urlParts[urlParts.length - 1];

            // Retrieve file via blob endpoint
            const retrieveRes = await dbApp.request(`/blob/${storedKey}`, { method: 'GET' }, dbEnv);

            expect(retrieveRes.status).toBe(200);
            expect(retrieveRes.headers.get('content-type')).toBe('text/plain');
            expect(await retrieveRes.text()).toBe('test content');
        });
    });
});
