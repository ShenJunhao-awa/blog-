...
export const files = sqliteTable("files", {
    id: integer("id").primaryKey(),
    key: text("key").notNull(),           // 文件唯一标识符
    filename: text("filename").notNull(), // 原始文件名
    mimeType: text("mimetype").notNull(), // MIME类型
    content: text("content").notNull(),   // 文件内容(base64编码)
    size: integer("size").notNull(),      // 文件大小(字节)
    uid: integer("uid").references(() => users.id, { onDelete: 'cascade' }), // 所属用户
    createdAt: created_at,
    updatedAt: updated_at,
});
